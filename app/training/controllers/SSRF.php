...
class SSRF
{
    ...
    public static function lesson6(){
        ...
        if( isset($_POST["url"]) ){
            ...
            if( substr($_POST["url"],0,7) == 'http://' || substr($_POST["url"],0,8) == 'https://' ){
                if( $url_sp[2] == 'localhost' || $url_sp[2] == 'thehack.lab' || $url_sp[2] == '169.254.169.254' || $url_sp[2] == '142.93.35.49' || substr($url_sp[2],0,4) == '127.' ){
                    $continue = false;
    ...
    public static function lesson5(){
        ...
        if( isset($_POST["url"]) ){
            ...
            $continue = true;
            if( substr($_POST["url"],0,7) == 'http://' || substr($_POST["url"],0,8) == 'https://' ){
                if( $url_sp[2] == 'localhost' || $url_sp[2] == 'thehack.lab' || $url_sp[2] == '142.93.35.49' || substr($url_sp[2],0,4) == '127.' ){
                    $continue = false;
                    $data["error"] = 'Only remote URLs are allowed';
                }
            }else{
    ...
    public static function lesson4(){
        ...
        if( isset($_POST["url"]) ){
            ...
            if( substr($_POST["url"],0,7) == 'http://' || substr($_POST["url"],0,8) == 'https://' ){
                $url_sp = explode("/",$_POST["url"]);
                if( $url_sp[2] == 'thehack.lab' || substr($url_sp[2],-18,18) == '.thehack.lab' ){
                }else{
                    $continue = false;
                    $data["error"] = 'Only URLs from the thehack.lab domain are allowed!';
                }
            }else{
                $continue = false;
                $data["error"] = 'URL must start with http:// or https://';
